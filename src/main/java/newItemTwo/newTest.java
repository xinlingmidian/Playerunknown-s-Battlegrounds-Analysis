...
public class newTest {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		Job job = Job.getInstance(conf, "newItemTwo");
		
		job.setJarByClass(newItemTwo.newTest.class); //设置执行任务的主类
		job.setMapperClass(newItemTwoMap.class);    //设置map的类
		job.setCombinerClass(newItemTwoReduce.class); // 因为hadoop是一个分布式，所有它会在每个节点进行一个预处理
		job.setReducerClass(newItemTwoReduce.class);  //设置Reduce 类 ，最终的结果输出
...
//==============    项目数据运行：   ============================		(10mins)
		...
		//FileInputFormat.setInputPaths(job, new Path("hdfs://192.168.147.168:9000/itemTwo-result1"));
		FileInputFormat.setInputPaths(job, new Path("hdfs://192.168.147.168:9000/itemTwo-result1"));
		 //路径中的目录必须不存在，需要它自己创建，否则会提示错误
		FileOutputFormat.setOutputPath(job, new Path("hdfs://192.168.147.168:9000/itemTwoResult2"));
